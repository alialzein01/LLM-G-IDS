...
| B5 | The loop is NOT separated from the head alone it consults: −0.0032 [−0.0228,+0.0128] UNSW, −0.0032 [−0.0342,+0.0299] ToN, sign unstable across seeds; head alone has the higher mean on both. | `multi_seed.comparisons.feedback_vs_head_alone`, `loop_vs_head_alone_headline` `[verified]` | **REPORTED** — decision reversed 2026-09-21. §4.2 gives the head's standalone scores, its separated margin over the GNN model, and this non-separation; §5.4 reads it against the isolation experiment. It is reported in prose and, following the supervisor revision of 2026-09-23, in the separate model/component metrics table. It is not a ladder row, because the head is a component the loop contains rather than a parallel stage. Earlier interval values in this row were stale and have been refreshed from the contract |
