...
\begin{table}[htbp]
\centering
\caption[Five prototype-consultant treatments]{Five treatments applied to the embedding-only consultant. None was kept. Rows marked
...
\midrule
Selector-head supervision & the ranking head is trained as a classifier & 3 seeds: feedback model worse on both datasets & no \\
Consultant temperature & per-fold calibration instead of a pinned $T$ & 3 seeds: feedback model worse, jointly with the above & no \\
