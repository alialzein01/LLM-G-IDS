...
$+0.1091$] on NF-ToN-IoT, which is not. The seed-matched interval on NF-ToN-IoT does exclude
zero, at $+0.0509$ [$+0.0022$, $+0.1013$], which is the same pattern AGAF against the graph
...
Figure~\ref{fig:comparisons} draws the same intervals. The table divides cleanly by consultant. Under the prototype scorer, \emph{nothing is separated
...
what each was worth, and Appendix~\ref{app:negative} gives the conditions behind the first
two in full.
...
loop actually consults, which Appendix~\ref{app:complementarity} tabulates. On the flagged set at
...
against 22 on NF-UNSW-NB15 and 129 against 75 on NF-ToN-IoT, and inside the confidence-gated
...
0.4141. The prediction and the inversion agree, and Figure~\ref{fig:ceiling} shows the mechanism
