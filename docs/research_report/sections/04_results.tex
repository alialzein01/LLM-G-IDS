...
\caption{The four rungs on both datasets, with the loop drawn under each of its two consultants. Error bars are one standard deviation over three training seeds at a fixed fold partition; fold-partition variance is not measured. Open circles are the three individual seeds. The semantic rung is deterministic and carries no spread. The hatched bar is the superseded prototype-consultant loop, for which per-seed values are reported but no interval was measured. The two panels have independent y-axes and their absolute heights are not comparable.}
\label{fig:ladder}
\end{figure}
...
loop actually consults, which Appendix~\ref{app:complementarity} tabulates. On the flagged set at seed 42 the graph encoder is 0.603 accurate on
