...
estimates. The distance between a controlled comparison and an operational detector is the
subject of \citet{sommer2010closedworld}, and nothing measured below closes it.
...
the two graphs. Ranked by mean macro-F1, highest first, NF-UNSW-NB15 gives the fusion model,
the GNN model, the semantic model. NF-ToN-IoT gives the GNN model, the fusion model, the
semantic model, so the fusion model falls below the rung it is meant to improve on. The semantic model is the weakest on both, and by
a much wider margin under NF-ToN-IoT's imbalance, where the prototype scorer reaches 0.2785
against the GNN model's 0.4336.

Table~\ref{tab:metrics} adds accuracy and support-weighted F1 beside macro-F1 for the same
rungs, scored on the same rows. The feedback model with its trained-head consultant is the
highest rung on all three metrics on both datasets, which macro-F1 alone does not establish.
The three metrics do not carry the same information, and NF-ToN-IoT shows why. Its benign class
holds 82\% of the scored edges, so every graph rung reaches at least 0.83 accuracy there while
their macro-F1 values run from 0.4102 to 0.5044. The feedback model stands above the GNN model
on both, at 0.5044 against 0.4336 on macro-F1 and 0.9186 against 0.8739 on accuracy, so the gap
that looks nearly closed is the one measured on a metric where the benign class casts four votes
in five. Weighted F1 behaves the same way for the same reason. Macro-F1 is the headline metric throughout this report because it
gives a 12-edge class and a 1{,}746-edge class the same weight, which is the comparison the
...
Because the consultant turned out to carry the change, it was also scored standing alone. The
...
cannot weigh it without knowing where the consultant sits on its own. It is not tabulated as a
rung, because it is a component the feedback model contains rather than a parallel stage of the
ladder, and Section~\ref{sec:disc-contribution} reads it against the isolation experiment.
...
\caption{Every ladder comparison as an interval, under each consultant. A difference counts as separated only when the interval excludes zero, shown by a filled marker. The primary two-level procedure resamples edges and training seed; the seed-matched procedure resamples edges alone. Under the trained head every feedback model comparison clears zero on both datasets; under the prototype consultant only the comparison against the semantic model on NF-ToN-IoT does. Colours follow the rung the comparison is about, with the prototype group in a lighter tint.}
\label{fig:comparisons}
\end{figure}

Figure~\ref{fig:comparisons} plots them. The table divides cleanly by consultant. Under the
...
sensitive to what the advice says and not merely to the presence of an extra tensor. Random
advice is not uniformly worse than none, however. On NF-ToN-IoT at seed 42 it reaches 0.4002
against \texttt{head\_only}'s 0.3809, the one cell of six where that happens. The claims here
are real advice against each of the other two arms, and both hold at every seed. This
...
The shares quoted below are taken against the headroom as a plain three-seed mean, $+0.0345$ on
NF-UNSW-NB15 and $+0.1090$ on NF-ToN-IoT. Those two values differ slightly from the point
estimates just given, which are bootstrap means over resampled edges and seeds.
...
That null is not an inert channel. Churn, the fraction of edges whose predicted class changes
...
alive and moves predictions on every pass. What it does not do is move the pooled score away from
control once output fusion is removed. The churn figures are in
...
$k = 35$, and the feedback model against the fusion model includes zero in all three
conditions.
...
Where the edge channel of Section~\ref{sec:res-ablations} changes roughly one prediction in eight
between passes, the alternative injection design, which biases graph attention rather than the
edge representation, produces churn of exactly 0.0000 in every configuration tested, including when handed the true
...
Macro-F1 averages over classes of very different sizes, so it hides which classes a rung is
good at. Figure~\ref{fig:perclasslevels} gives the levels: four rungs on every evaluated class,
ordered by edge count. Table~\ref{tab:perclass} in Appendix~\ref{app:perclass} gives the same
values with the counts beside them. Two patterns are visible before any difference is taken.
Performance falls with support on both datasets, and it falls much faster on NF-ToN-IoT, where
the four smallest classes hold 12 to 25 edges and no rung reaches 0.35 on three of them.
Figure~\ref{fig:perclass} then gives the differences the Discussion argues about.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{fig_perclass_levels}
\caption{Per-class F1 for the four rungs, both datasets, with classes ordered by edge count and the count under each label. The feedback model is the canonical one, consulting the trained head. Error bars are one standard deviation over three training seeds at a fixed fold partition. No per-class value carries an interval, and on the smallest classes a single edge moves F1 substantially.}
...
The feedback model's gains are concentrated in the low-support classes, which is where a
macro-average is decided. Against the GNN model it adds $+0.2090$ on NF-UNSW-NB15's Backdoors
and $+0.1594$ on its DoS, both 40-edge classes, and $+0.0903$ on NF-ToN-IoT's \texttt{backdoor}
at 16 edges and $+0.2242$ on \texttt{scanning} at 13. Its losses are in the same size range and
go the other way: $-0.0273$ on \texttt{mitm}, $-0.1116$ on \texttt{xss} at 12 edges, and
\texttt{password} at 0.2134 against the fusion model's 0.2996. On classes this small a single
edge moves F1 by several points, so these are the differences least able to bear weight, and
none of them carries an interval.
