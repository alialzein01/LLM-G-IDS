...
\caption{The four rungs on both datasets, with the loop drawn under each of its two consultants. Error bars are one standard deviation over three training seeds at a fixed fold partition; fold-partition variance is not measured. Open circles are the three individual seeds. The semantic rung is deterministic and carries no spread. The hatched bar is the superseded prototype-consultant loop; the arrow between the two loop bars carries an interval, given in the text. The two panels have independent y-axes and their absolute heights are not comparable.}
\label{fig:ladder}
\end{figure}

Figure~\ref{fig:ladder} draws the same numbers. The three fixed rungs behave differently on
the two graphs. On NF-UNSW-NB15 they run AGAF,
...
zero, at $+0.0509$ [$+0.0022$, $+0.1013$]. That is the same pattern AGAF against the graph
encoder shows below: a difference that survives resampling edges and stops surviving once
...
\caption{Every ladder comparison as an interval, under each consultant. A difference counts as separated only when the interval excludes zero, shown by a filled marker. The primary two-level procedure resamples edges and training seed; the seed-matched procedure resamples edges alone. Under the trained head every loop comparison clears zero on both datasets; under the prototype consultant none does. Colours follow the rung the comparison is about, with the prototype group in a lighter tint.}
...
loop actually consults. Appendix~\ref{app:complementarity} tabulates that. On the flagged set at
seed 42 the graph encoder is 0.547 accurate on NF-UNSW-NB15 and 0.343 on NF-ToN-IoT. The
...
edges where the consultant and the graph encoder disagree, the trained head is right 55 times
...
\caption{Per-class differences between the loop and the two rungs below it, three-seed means, with class sizes in brackets. The scale is centred at zero and a black outline marks a loss. No per-class difference carries an interval, and on classes of 12 to 40 edges a single edge moves F1 substantially.}
\label{fig:perclass}
\end{figure}
...
RQ4 turns on two rare-class observations. AGAF's NF-ToN-IoT collapse is concentrated in the
