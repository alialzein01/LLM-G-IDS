...
to one, so the balance between structure and semantics is decided separately for every edge
...
gives a Majorization--Minimization argument, the technique of repeatedly minimising an easier
upper bound in place of a hard objective, under which its refinement loop monotonically
...
with dropout left switched on, to identify uncertain nodes. It then prompts the LLM with node
text and one-hop topology and receives a predicted label together with a free-text explanation
