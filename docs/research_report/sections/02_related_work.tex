...
Intrusion detection is conventionally divided into signature-based and anomaly-based
approaches \citep{scarfone2007idps}, but either can be posed over a flow graph, and a flow
...
IP addresses; edges represent flows or aggregated flows. This formulation preserves the
...
the edge's own features, and the model is paired with SHAP-based explanation
\citep{tegsage2025}. This changes the representational question. Once the edge itself is
...
retaining both endpoint structure and edge-specific content. The next question is whether
edge content has to remain numeric, or whether some of it can be represented semantically.
...
is a BERT-class encoder adapted to cybersecurity text \citep{bayer2022cysecbert}. Its
...
measurements. Together they support a semantic branch in which a flow description is
encoded as text rather than treating every field as a scalar magnitude. They do not, by
...
The simplest family is late fusion with a fixed interface between branches. GMLM uses
separate graph and language towers, concatenates their representations, and passes the
result to an MLP \citep{sinha2025gmlm}; its own comparison explicitly distinguishes this
design from cross-attention, gating, and learned modality weights. BertGCN gives an even
cleaner example of fixed weighting: its final prediction linearly interpolates a GCN
prediction and a BERT prediction using a global hyperparameter \citep{lin2021bertgcn}. Every
sample receives the same interpolation coefficient. These methods keep the encoders modular,
but the fusion rule never decides that one modality should matter more for one input and
less for another.
...
side \citep{beiranvand2025bigtex}. Attention-based interaction differs from global
interpolation because its interaction weights are derived from the representations of the
current input.
...
mix the modalities repeatedly rather than preserving two independent representations until
classification.
...
globally fixed or varies with the sample. The second distinction becomes sharper on moving
...
explanations \citep{farrukh2024xgnid}. The system's ``dual modality'' refers to flow-level
and packet-level data inside the graph pipeline, not to graph and text embeddings fused for
classification.

Agent-based analysis for GNN robustness reverses the order.  % numcheck: allow -- "robustness" names RoGRAD's own topic, not a claim about our system An LLM agent analyses textual
descriptions of node interactions and flags, removes, or reweights suspicious nodes and
edges before the GNN processes the graph \citep{zhan2025agentgnn}. The language component
...
The broader GNN--LLM literature already contains the feedback mechanisms this report builds
on. They are prior work and are not claimed here as contributions.
...
implicit supervision that steers semantic refinement, and the refined semantics are fed back
to update the same graph learner \citep{thapaliya2025das}. DAS also constructs node
descriptions by expressing structural statistics in natural language --- degree,
betweenness, closeness, clustering coefficient, and square clustering coefficient --- and
gives a Majorization--Minimization argument under which its refinement loop monotonically
decreases a task-adaptive surrogate objective. Its setting is node-level, over general
text-rich graphs. Closed graph--language feedback, and the conversion of structural
statistics into natural-language descriptions, therefore already exist in the literature.

LOGIN already couples uncertainty selection to language-model consultation. It uses
MC-dropout variance to identify uncertain nodes, prompts the LLM with node text and one-hop
topology, and receives a predicted label together with a free-text explanation
\citep{qiao2024login}. Incorporation depends on correctness: if the LLM's label matches the
...
Selecting uncertain graph elements for semantic consultation is therefore also established.
...
deficiencies including noise, sparsity, and label scarcity, and its abstract claims priority
for moving language-model augmentation of graphs from static injection to an iterative
refinement paradigm. Iterative semantic refinement is therefore already an explicit
contribution claim in the prior literature.

These four anchors settle the component question plainly. Closed GNN--language feedback
already exists. Iterative semantic refinement already exists. Structural statistics have
already been expressed as natural-language input. Uncertainty and learned routing have
already been used to decide which graph elements receive language-model consultation. Taken
component by component, the feedback mechanism is not the contribution of this report.

\subsection{Positioning}

The contribution is an edge-level intrusion-detection instantiation, and a measurement
question about how such a feedback channel is used. The comparison therefore has to be made
at the level of configuration rather than of individual mechanisms.
...
encoder whose embeddings are scored against class prototypes. Their feedback actions refine
...
\citep{qiao2024login,thapaliya2025das,loveland2025glance,wang2025rograd}.

Two signals travel in opposite directions in the resulting loop, and it is worth stating
precisely which. The graph model's per-edge uncertainty determines \emph{which} edges are
...
representations. The coupling is bidirectional in this routing sense. It is not bidirectional
in content: the consultant's verdict on an edge depends on that edge's text alone, not on
what the graph model currently believes about it.

The final classification stage adds a further dimension to the configuration. Structural and
semantic edge representations are combined through joint attention-based fusion with
...
\citep{li2024ragformer,beiranvand2025bigtex,lee2025cast}. None of those mechanisms is a
...
coupling between a graph model and a semantic branch, joint attention-based fusion for the
...
\citep{qiao2024login,thapaliya2025das,loveland2025glance,wang2025rograd}. The present work
instead instantiates the loop in network-flow intrusion detection with a deliberately more
