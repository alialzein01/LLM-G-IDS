...
This report investigates whether structural and semantic evidence can be combined in a single edge-level intrusion detection framework. Each classified edge represents an aggregated group of flows between hosts. We evaluate the same pipeline on NF-UNSW-NB15 and NF-ToN-IoT. The scores are controlled comparisons under the graph-construction assumptions explained in Section~\ref{sec:method-graph}; they are not deployment estimates.
...
LLM, returns the revised semantics to that same graph learner, and writes each node's
structural statistics out as a sentence. Its setting is node-level, and it is evaluated on
graphs with and without node text~\citep{thapaliya2025das}. The
...
and a semantic branch, sample-adaptive gated fusion for the final classification, and
...
The report is organised into six sections, followed by three appendices. This Introduction
defines the problem, experimental scope, research questions, and contribution.
...
Section~\ref{sec:results} reports the predefined model ladder, baseline comparisons,
ablations, and mechanism-isolation measurements for both NF-UNSW-NB15 and NF-ToN-IoT, while
keeping the different scored class sets explicit. Section~\ref{sec:discussion} interprets those
...
intrusion detection. Section~\ref{sec:conclusion} states what the study established, what it
did not, and what remains open. The appendices hold the negative mechanism results in full,
the supporting per-class and knob tables, and the reproducibility record.
