...
from the model's input features and generated text. The consequence is explicit: the
...
distribution is far more skewed, with benign traffic dominant and most attack classes
represented by a few dozen edges or fewer. By the most-to-least class ratio, the two
...
RQ4 already asks how class imbalance affects fusion and feedback; the two-dataset design
...
domain-specific representation and classification tasks~\citep{bayer2022cysecbert}. It is
...
instead to let them challenge and complement one another. The parallel fusion stage runs
the graph and semantic encoders side by side and combines their edge embeddings through
Adaptive Gated Attention Fusion (AGAF). The feedback stage adds uncertainty-guided
...
and is reported as a verified negative result.
...
\citep{farrukh2024xgnid}. DAS is the closest prior work to the feedback mechanism
...
\begin{description}
    ...
    NF-ToN-IoT, and are the observed differences supported by uncertainty estimates and
    robust comparisons?
...
\begin{enumerate}
    ...
    evidence into the edge representations of uncertain edges. The feedback loop consults
    a classification head trained on the semantic embeddings; that head, evaluated alone,
    is reported beside the loop as the strongest single-modality baseline, so the reader
    can see whether the loop adds to its own consultant or only repeats it.
...
headline of that evidence is stated here so it is not mistaken for a footnote later: the
feedback loop is the strongest system measured on both datasets, separated from the graph
model, the fusion model and every re-trained baseline, and it is statistically
indistinguishable from the semantic head it consults. The feedback channel is live and
measurable; what it adds in accuracy beyond its consultant is, on these graphs, nothing we
could separate from zero.
...
datasets, graph and text construction, the structural and semantic branches, AGAF,
