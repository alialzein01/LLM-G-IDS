...
single edge-level intrusion detection framework, meaning that the unit being classified is an
edge: one aggregated group of flows between a pair of hosts. The two datasets are
NF-UNSW-NB15 and NF-ToN-IoT. One experimental boundary must be stated from the outset:
the current graph construction
...
from the GNN model, from the fusion model, from the semantic model and from every faithful
re-trained baseline. With the weaker consultant it was first built with, neither the comparison
against the GNN model nor the one against the fusion model is separated on either dataset, and
the margin over E-GraphSAGE on NF-ToN-IoT disappears as well.
...
The channel is live and measurable, but on these graphs what the re-entry itself adds is
indistinguishable from adding nothing.
