...
One constraint on this stage is load-bearing. The fusion model consumes \emph{per-fold out-of-fold}
...
could detect. For each held-out fold, the structural embeddings supplied to the fusion model come from a
...
The embedding-only alternative, used by the semantic model and by the fusion model, is a whitened
...
reported alone. The feedback model was first built with the prototype scorer as its consultant, on the
