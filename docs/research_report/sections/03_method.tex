...
directed graph; degree is \emph{in-degree} centrality; $k$-core and $k$-truss are computed on
...
projection from the 10 input channels to 64; the second's is an identity. The final node
...
normalisation and ReLU --- 64 to 128 for the structural embedding, 768 to 128 for the semantic
...
the fusion classifier through its own input features --- a leak that no split downstream of it
...
target. We select $k$ on validation folds only: $k = 31.0$ on NF-UNSW-NB15, $k = 25.0$ on
NF-ToN-IoT.

\paragraph{Consultation.} The semantic consultant is a whitened prototype scorer. Within each
fold, we centre the training-fold semantic embeddings, fit a ZCA whitener by eigendecomposition
of their covariance with a floor on small eigenvalues --- necessary because the training fold
has fewer rows than the 768 embedding dimensions --- and take the class-wise mean in the
whitened space as that class's prototype. At inference an edge is whitened with the same
fold-local transform and scored by cosine similarity against the prototypes. Whitening is not
incidental: the earlier L2-normalise-average-renormalise scheme collapsed per-class spread
until random and real advice became indistinguishable. Every rung uses this same scorer; none
uses a trained supervised head, which would make the loop echo a classifier it already
contains.
...
effective feedback rate is thus $15.5\%$ of edges on NF-UNSW-NB15 and $12.5\%$ on NF-ToN-IoT.
...
validation folds. Because $\tilde{\mathbf{x}}_{e}$ enters both attention and readout, the
...
signal must land in order to have any effect at all, which is a question the positive result
...
--- 4999 epochs for E-GraphSAGE, 20 for TE-G-SAGE --- which is incompatible with a protocol
...
one IP pair are mathematically indistinguishable to it --- 58.7\% of NF-UNSW-NB15 edges and
...
Model selection uses validation folds exclusively. The uncertainty threshold $k$ was selected
by a validation sweep over $[15, 35]$ averaged across three seeds (42, 1, 2); the remaining
...
class count; classes absent from a training fold receive weight zero. Focal loss down-weights
