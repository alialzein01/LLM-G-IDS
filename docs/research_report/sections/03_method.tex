...
One internal baseline sits beside the two published ones in every table: the trained
semantic head alone, scored out-of-fold exactly as the ladder rungs are. It is the loop's own
consultant evaluated without the graph, and it is the number the loop must be read against;
a system that merely repeats its strongest component would tie it. It is trained at the same
three seeds so that its comparison with the loop carries seed variance on both sides.
