...
\caption[Pipeline architecture and evaluated stages]{The pipeline, from flow records to four scored rungs. Blue is the structural branch, orange the semantic branch, green the fusion stage and vermillion the feedback model. Every rung is an exit point from one shared graph and is scored by one protocol on one fold partition and one set of seeds. The marked aggregation step is the label-conditioned grouping key that bounds every claim in this report.}
...
The fusion model projects each modality into a shared 128-dimensional space through a linear layer, batch
...
This is the gated multimodal unit's feature-wise form \citep{arevalo2017gmu} rather than a single
