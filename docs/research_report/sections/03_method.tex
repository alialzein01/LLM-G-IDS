...
\caption{The pipeline, from flow records to four scored rungs. Blue is the structural branch, orange the semantic branch, green the fusion stage and vermillion the feedback loop. Every rung is an exit point from one shared graph and is scored by one protocol on one fold partition and one set of seeds. The marked aggregation step is the label-conditioned grouping key that bounds every claim in this report.}
...
number. That head is the consultant the feedback loop uses. Evaluated on its own, it is
...
own judgement is a floor the loop can fall back to on edges where the graph view is
...
the semantic consultant. The loop was first built around the whitened prototype scorer, the same
one the semantic rung uses, and later around a per-fold classification head trained on
