...
\caption{The pipeline, from flow records to four scored rungs. Blue is the structural branch, orange the semantic branch, green the fusion stage and vermillion the feedback model. Every rung is an exit point from one shared graph and is scored by one protocol on one fold partition and one set of seeds. The marked aggregation step is the label-conditioned grouping key that bounds every claim in this report.}
...
encoded edge vector, given in Table~\ref{tab:focus}. These nine weights were fixed a priori and
never swept. They are source-code constants, not selected values. The protocol weight is broadcast across
...
mean in a fold-local whitened space. This is the ladder's semantic model, and it measures what
the encoder alone knows; the fusion model takes the same embedding with no scorer on top. The prototype
is the semantic model because it fits nothing, not because it is cheap. A nearest-neighbour
readout over the training-fold embeddings would cost nothing extra on graphs of 656 and
2{,}127 edges, but its answer for an edge would depend on which training edges happen to lie
closest to it and on a neighbourhood size that would itself need selecting. Ten class means
add no classifier of their own, so the score reports how well the frozen encoder separates the
classes, which is also why the semantic model's score does not vary with the training seed in
Section~\ref{sec:results}. The \emph{trained
head} route fits, within each fold, a small classifier on the training-fold embeddings: a
per-fold standard scaler, then a 768--128--128--$C$ multilayer perceptron, where $C$ is the
number of classes and equals ten on both graphs, with batch normalisation and ReLU in the
projection, ReLU and dropout $0.2$ in the hidden layer, trained with the same class-weighted
focal loss, optimiser and early-stopping rule as the structural branch. Each fold's head is applied to every edge and its logits are
...
number. That head is the consultant the feedback model uses. Evaluated on its own, it is
also the strongest single-modality baseline in this study, and this report treats it as the
consultant rather than as a rung of the ladder. A cross-fitted variant, in which training-edge logits come from an inner
cross-validation so that the feedback model trains on advice no more reliable than the advice it is
scored with, was implemented and measured. It scored lower on both datasets and is reported
in Section~\ref{sec:results}. The in-fold head is retained.
...
One constraint on this stage is load-bearing. The fusion model consumes \emph{per-fold
...
What the construction guarantees is worth stating precisely, because it is weaker than
\emph{never}. For each held-out fold, the embeddings of that fold's \emph{test} edges come from
...
The rule that keeps the comparison fair applies everywhere: every
rung uses the same encoder, the same graph, the same folds and the same training seeds. The
feedback model additionally trains a classification head on the semantic embeddings, and that head is
its consultant. The feedback model was first built with the prototype scorer as its consultant, on the
argument that a consultant the feedback model already contains would only be echoed. That
configuration, and five attempts to make its advice usable, are reported as negative results
in Section~\ref{sec:results}. The trained head replaced it after those measurements. Both
configurations are reported, at the same three seeds, at the same fold partition, and with the
injection scale unchanged. Only the selected entropy percentile differs between them, at 31
against 29 on NF-UNSW-NB15 and 25 against 16 on NF-ToN-IoT.
...
own judgement is a floor the feedback model can fall back to on edges where the graph view is
unsure. This stage has the same projection width as the fusion model, so the feedback model and the fusion model differ in
