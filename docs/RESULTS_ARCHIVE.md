...
**Why this file exists:** `CLAUDE.md` is loaded into every agent context window, so it must
...
is a duplicate; `CLAUDE.md` carries the *rules and traps*, this carries the *record*.
...
`CLAUDE.md` carried, for some time: *"Oracle mechanism ceiling on UNSW: +0.0508 (control 0.7237
...
Caught by Codex review. **Do not reintroduce those numbers.**
...
a separate, stronger baseline and never the ladder's LLM rung (CLAUDE.md, §5).
